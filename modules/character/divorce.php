...
$title = Athena::message('DivorceTitle');
...
if (!$char->partner_id) {
	$session->setMessageData(sprintf(Athena::message('DivorceNotMarried'), $char->name));
	$this->redirect($this->referer);
}

$partner = $server->getCharacter($char->partner_id);
if (!$partner) {
	$session->setMessageData(Athena::message('DivorceInvalidPartner'));
	$this->redirect($this->referer);
}

$child = false;
if ($char->child && !($child=$server->getCharacter($char->child))) {
	$session->setMessageData(Athena::message('DivorceInvalidChild'));
	$this->redirect($this->referer);
}

if ($char->online || $partner->online || (!Athena::config('DivorceKeepChild') && $child && $child->online)) {
	$session->setMessageData(sprintf(Athena::message(Athena::config('DivorceKeepChild') ? 'DivorceMustBeOffline' : 'DivorceMustBeOffline2'), $char->name));
	$this->redirect($this->referer);
}

if (count($_POST) && $params->get('divorce')) {
	$sql = "UPDATE {$server->charMapDatabase}.`char` SET partner_id = 0 ";
	if (!Athena::config('DivorceKeepChild')) {
		$sql .= ", child = 0 ";
	}
	...
	$session->setMessageData(sprintf(Athena::message('DivorceSuccessful'), $char->name));
	$this->redirect();
}
?>